  ...
  return (
    <div className='col-span-2 h-[calc(100vh-96px)] overflow-y-scroll scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-200 rounded-md'>
      <article className='flex items-center justify-between px-10 py-5 '>
        <div className='typography dark:bg-black p-5 rounded-2xl border border-slate-500'>
          <h2 className='text-4xl'>Savitar.dev - dasturlash kurslari</h2>
          <button
            onClick={() => selector('Kurslar' , '/courses')}
            className='py-2 px-3 mt-4 mr-2 dark:bg-white bg-slate-800 text-white dark:text-slate-800 rounded-xl '
          >
            Kurslar
          </button>
          <button onClick={() => selector('Loyihalar' , '/projects')} className='py-2 px-3 mt-4 dark:bg-white bg-slate-800 text-white dark:text-slate-800 rounded-xl '>
            Loyihalar
          </button>
        </div>
        <Slider />
        ...
        {course.map(
          (course, id) =>
            id <= 5 && (
              <div
                key={course.title}
                className='dark:bg-[#19191c] rounded-lg border border-slate-600'
              >
